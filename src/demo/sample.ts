...
// participial-closer split (tense-agreeing), the additive correlative
// collapsed to a conjunction ("not only X but also Y" -> "X and Y"), a
// ladder substitution, and three things left for the residual suggest
// lines: an idiom the span deletion now declines to split ("Simply put"),
// a corrective contrast flagged for a human rewrite ("isn't X; it's Y"),
// and a jargon metaphor.

export const SAMPLE_INPUT =
  "This guide will walk you through configuring the backup agent. The agent performs validation of the config file, then utilizes the snapshot catalog in order to locate missing segments. Simply put, a restore replays the catalog. v2 replaced the retry loop, allowing the agent to resume after a crash. The agent is not only fast but also safe to interrupt. The slow part isn't the upload; it's the checksum pass. This will streamline their deployment and untangle the dependency jungle. It is important to note that offsets should match before you deploy. If you have any questions, please reach out to our support team.";

export const SAMPLE_OUTPUT =
  "This guide covers configuring the backup agent. The agent validates the config file, then uses the snapshot catalog to locate missing segments. Simply put, a restore replays the catalog. v2 replaced the retry loop. That allowed the agent to resume after a crash. The agent is fast and safe to interrupt. The slow part isn't the upload; it's the checksum pass. This will simplify their deployment and untangle the dependency jungle. Offsets should match before you deploy.";

export const SAMPLE_PASS_COUNT = 4;
export const SAMPLE_PATCH_COUNT = 10;
